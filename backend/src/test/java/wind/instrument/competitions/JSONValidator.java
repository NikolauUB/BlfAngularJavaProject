...
public class JSONValidator {
            ...
            throws ProcessingException
    {
        ...
        if (!report.isSuccess()) {
            for (ProcessingMessage processingMessage : report) {
                //report.debug(processingMessage);
                throw new ProcessingException(processingMessage);
            }
        }
    } // validateJson(Node) ends

    public static boolean validateJsonWithLogging(JsonSchema jsonSchemaNode,
                                                  JsonNode jsonNode,
                                                  Logger logger)
            throws ProcessingException
    {
        ...
        if (!report.isSuccess()) {
            for (ProcessingMessage processingMessage : report) {
                logger.error(new ProcessingException(processingMessage).getMessage());
            }
        }
    ...
    public static boolean isJsonValid(String schemaText, String jsonText, Logger logger) throws ProcessingException, IOException
    {
        final JsonSchema schemaNode = getSchemaNode(schemaText);
        final JsonNode jsonNode = getJsonNode(jsonText);
        return validateJsonWithLogging(schemaNode, jsonNode, logger);
    } // validateJson(Node) ends
